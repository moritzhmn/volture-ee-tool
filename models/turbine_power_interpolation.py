...
df['rated_power_kw'] = df['rated_power_mw'].astype(float)
...
def get_turbine_power_value(target_mw, wind_speed, case):
    ...
    total_outputs = curves.sum(axis=1)
...
# === Kurven für Plot außerhalb der Funktion laden ===
def get_case_curves(target_mw):
    target_kw = target_mw * 1000
    class_idx = np.digitize([target_kw], bins)[0] - 1
    selected_class = labels[class_idx]
    class_df = df[df['class'] == selected_class]
    ...
    return {
        ...
        'best': best_curve.values,
        'normal': avg_curve.values,
        'worst': worst_curve.values
    }
